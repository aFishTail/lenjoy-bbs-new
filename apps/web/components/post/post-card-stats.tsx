...
export function PostCardStats({ viewCount, commentCount, likeCount, createdAt }: Props) {
  ...
  return (
    <div className={styles.stats}>
      <span className={styles.stat} title="浏览量">
      ...
      <span className={styles.stat} title="评论数">
        <svg className="icon-sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
        </svg>
        <span>{commentCount || 0}</span>
      </span>
      <span className={styles.stat} title="点赞数">
        <svg className="icon-sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d="M7 10v10" />
      ...
      {createdDate ? (
        <span className={`${styles.stat} ${styles.date}`} title="发布时间">
          <svg className="icon-sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
