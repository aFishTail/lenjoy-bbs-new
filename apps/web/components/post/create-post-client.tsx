...
const postTypeOptions = [
  { value: "NORMAL", label: "普通帖子", icon: "M19 20H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v1m2 13a2 2 0 0 1-2-2V7m2 13a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2", desc: "分享想法、讨论问题" },
  { value: "RESOURCE", label: "资源出售", icon: "M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5", desc: "出售文档、源码、素材" },
  { value: "BOUNTY", label: "悬赏求助", icon: "M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z", desc: "付费寻求帮助解答" },
] as const;
...
export function CreatePostClient() {
  ...
  const [postType, setPostType] = useState<"NORMAL" | "RESOURCE" | "BOUNTY">("NORMAL");
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const request: CreatePostRequest = {
        ...
        hiddenContent: hiddenContent.trim() || undefined,
  ...
  if (!auth) {
    return (
      <main className="page">
        <div className="text-center py-12 text-[var(--text-sub)]">正在跳转到登录页...</div>
      </main>
    );
  }
    ...
    <main className="page min-h-[calc(100vh-80px)] py-8">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-[var(--text-main)] mb-2" style={{ fontFamily: "'Newsreader', serif" }}>
            发布新帖子
          </h1>
        ...
        <form onSubmit={(event) => void onSubmit(event)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {postTypeOptions.map((option) => (
                ...
                >
                  ...
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${postType === option.value ? "bg-[var(--color-primary)] text-white" : "bg-[var(--border-light)] text-[var(--text-sub)]"}`}>
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d={option.icon} />
                    </svg>
                  </div>
                  <div className="font-medium text-[var(--text-main)]">{option.label}</div>
          ...
          {/* 标题 */}
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-[var(--border-light)] shadow-[var(--shadow-md)]">
            <label className="block text-sm font-medium text-[var(--text-sub)] mb-3" htmlFor="title">
          ...
          {/* 内容 */}
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-[var(--border-light)] shadow-[var(--shadow-md)]">
            <label className="block text-sm font-medium text-[var(--text-sub)] mb-3" htmlFor="content">
              内容 <span className="text-[var(--color-danger)]">*</span>
            </label>
            <textarea
              id="content"
              className="w-full px-4 py-3 rounded-xl border border-[var(--border-medium)] bg-white/50 text-[var(--text-main)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent transition-all resize-none"
              required
              rows={8}
          ...
          {postType === "RESOURCE" && (
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-[var(--border-light)] shadow-[var(--shadow-md)] space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-sub)] mb-2" htmlFor="hiddenContent">
                  付费内容（购买后可见）
                </label>
                <textarea
                  id="hiddenContent"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-sub)] mb-2" htmlFor="price">
                  价格 (元) <span className="text-[var(--color-danger)]">*</span>
                </label>
          ...
          {postType === "BOUNTY" && (
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-[var(--border-light)] shadow-[var(--shadow-md)]">
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-sub)] mb-2" htmlFor="bountyAmount">
                  悬赏金额 (元) <span className="text-[var(--color-danger)]">*</span>
                </label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-muted)]">¥</span>
                  <input
        ...
        <div className="text-center mt-8">
          <a href="/" className="inline-flex items-center gap-2 text-[var(--text-sub)] hover:text-[var(--color-primary)] transition-colors">
