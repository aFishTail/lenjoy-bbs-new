...
from lenjoy_bbs.modules.posts import service as posts_service
...
async def test_register_user_rolls_back_when_wallet_adjustment_fails(monkeypatch):
    ...
    async def fail_adjust(db, user_id, *args, **kwargs):
        ...
        role = await db.scalar(select(UserRole).where(UserRole.user_id == user_id))
    ...
    monkeypatch.setattr(auth_service, "adjust_available", fail_adjust)
    ...
    async with SessionLocal() as db:
        user = await db.scalar(select(UserAccount).where(UserAccount.username == "rollback-user"))
        user_role = await db.scalar(select(UserRole).where(UserRole.user_id == observed["user_id"]))
        wallet = await db.scalar(select(Wallet).where(Wallet.user_id == observed["user_id"]))
        ledger = await db.scalar(select(WalletLedger).where(WalletLedger.user_id == observed["user_id"]))
...
@pytest.mark.asyncio
async def test_purchase_post_rolls_back_purchase_and_ledger_when_credit_side_fails(monkeypatch):
    async with SessionLocal() as db:
        seller = await _register_account(db, "seller-rollback", "seller-rollback@example.com")
        buyer = await _register_account(db, "buyer-rollback", "buyer-rollback@example.com")
    ...
    original_adjust_available = posts_service.adjust_available
    observed = {
    ...
    async def fail_credit_side(db, user_id, delta, biz_type, biz_key, remark, operated_by=None):
        if user_id == buyer.id and biz_type == "RESOURCE_PURCHASE":
            ...
            return await original_adjust_available(db, user_id, delta, biz_type, biz_key, remark, operated_by)
        ...
        if user_id == seller.id and biz_type == "RESOURCE_SALE":
            ...
            buyer_wallet = await db.scalar(select(Wallet).where(Wallet.user_id == buyer.id))
            buyer_ledger_staged = any(
                isinstance(obj, WalletLedger) and obj.biz_key == purchase_biz_key for obj in db.sync_session.new
            )
    ...
    monkeypatch.setattr(posts_service, "adjust_available", fail_credit_side)

    async with SessionLocal() as db:
        buyer = await db.scalar(select(UserAccount).where(UserAccount.username == "buyer-rollback"))
        assert buyer is not None

        with pytest.raises(RuntimeError, match="credit failed"):
            await posts_service.purchase_post(db, post.id, buyer)

    async with SessionLocal() as db:
        buyer = await db.scalar(select(UserAccount).where(UserAccount.username == "buyer-rollback"))
        seller = await db.scalar(select(UserAccount).where(UserAccount.username == "seller-rollback"))
        assert buyer is not None
        ...
        purchase_ledger = await db.scalar(select(WalletLedger).where(WalletLedger.biz_key == purchase_biz_key))
        sale_ledger = await db.scalar(select(WalletLedger).where(WalletLedger.biz_key == sale_biz_key))
        buyer_wallet = await db.scalar(select(Wallet).where(Wallet.user_id == buyer.id))
        seller_wallet = await db.scalar(select(Wallet).where(Wallet.user_id == seller.id))
...
async def test_purchase_post_checks_funds_on_locked_wallet(monkeypatch):
    async with SessionLocal() as db:
        seller = await _register_account(db, "seller-locked-wallet", "seller-locked-wallet@example.com")
        buyer = await _register_account(db, "buyer-locked-wallet", "buyer-locked-wallet@example.com")
        post = await posts_service.create_post(
            db,
            PostCreateRequest(
                type="RESOURCE",
    ...
    async def fail_adjust_available(*args, **kwargs):
        ...
        raise AssertionError("adjust_available should not be called when locked wallet lacks funds")

    monkeypatch.setattr(posts_service, "lock_wallet", fake_lock_wallet)
    monkeypatch.setattr(posts_service, "adjust_available", fail_adjust_available)

    async with SessionLocal() as db:
        buyer = await db.scalar(select(UserAccount).where(UserAccount.username == "buyer-locked-wallet"))
        assert buyer is not None
...
async def test_create_post_rolls_back_when_tag_validation_fails():
    async with SessionLocal() as db:
        author = await _register_account(db, "tag-rollback-author", "tag-rollback-author@example.com")
    ...
    async with SessionLocal() as db:
        post = await db.scalar(select(Post).where(Post.title == "Invalid tags create"))
...
async def test_update_post_rolls_back_when_tag_validation_fails():
    async with SessionLocal() as db:
        author = await _register_account(db, "tag-update-author", "tag-update-author@example.com")
        valid_tag_ids = list((await db.scalars(select(Tag.id).order_by(Tag.id.asc()).limit(1))).all())
        ...
        with pytest.raises(ApiError, match="One or more tags do not exist"):
            await posts_service.update_post(
                db,
                post_id,
                PostUpdateRequest(
    ...
    async with SessionLocal() as db:
        ...
        stored_tag_ids = list((await db.scalars(select(PostTag.tag_id).where(PostTag.post_id == post_id))).all())
