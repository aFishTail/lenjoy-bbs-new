...
def _iter_attribute_calls(tree: ast.AST, method_names: set[str]) -> Iterable[tuple[str, int]]:
...
def test_selected_routers_do_not_call_commit_or_rollback_directly():
    for path in ROUTER_FILES:
        tree = _load_ast(path)
        direct_calls = list(_iter_attribute_calls(tree, {"commit", "rollback"}))
...
def test_wallet_router_does_not_commit():
    content = (API_ROOT / "lenjoy_bbs/modules/wallet/router.py").read_text(encoding="utf-8")
...
def test_users_router_delegates_profile_writes_to_service():
    ...
    direct_calls = list(_iter_attribute_calls(tree, {"commit", "rollback", "flush"}))
...
def test_open_api_router_delegates_writes_to_service():
    ...
    assert "lenjoy_bbs.modules.open_api.service" in imports
...
def test_posts_router_delegates_writes_to_service():
    ...
    assert "lenjoy_bbs.modules.posts.service" in imports
    assert {"create_post", "update_post", "delete_post", "create_comment", "purchase_post"}.issubset(awaited_calls)
    assert direct_calls == []
...
def test_messages_router_delegates_writes_to_service():
    ...
    assert {"mark_message_read", "mark_all_messages_read"}.issubset(awaited_calls)
    assert direct_calls == []
...
def test_reports_router_delegates_writes_to_service():
    ...
    assert {"create_post_report", "create_comment_report"}.issubset(awaited_calls)
    assert direct_calls == []
