...
async def list_wallets(db: AsyncSession) -> list[dict]:
    ...
    return [model_dict(wallet, ["id", "user_id", "available_coins", "frozen_coins", "updated_at"]) for wallet in rows]


async def list_wallet_ledger(db: AsyncSession) -> list[dict]:
    rows = (await db.scalars(select(WalletLedger).order_by(WalletLedger.created_at.desc()).limit(100))).all()
    return [
        model_dict(row, ["id", "user_id", "direction", "change_amount", "balance_after", "biz_type", "created_at"])
        for row in rows
    ]


async def list_resource_trades(db: AsyncSession) -> list[dict]:
    rows = (await db.scalars(select(ResourcePurchase).order_by(ResourcePurchase.created_at.desc()).limit(100))).all()
    return [model_dict(row, ["id", "post_id", "buyer_id", "seller_id", "price", "status", "created_at"]) for row in rows]
...
async def update_wallet_coins(db: AsyncSession, user_id: int, amount: int, reason: str | None, admin_id: int) -> dict:
    ...
    return {"userId": user_id, "availableCoins": wallet.available_coins, "frozenCoins": wallet.frozen_coins}
