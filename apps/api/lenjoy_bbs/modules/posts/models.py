...
class Post(Base):
    ...
    id: Mapped[int] = mapped_column(IdType,
                                    primary_key=True,
                                    autoincrement=True)
...
class PostTag(Base):
    ...
    id: Mapped[int] = mapped_column(IdType,
                                    primary_key=True,
                                    autoincrement=True)
    post_id: Mapped[int] = mapped_column(ForeignKey("bbs_post.id"),
                                         nullable=False)
    tag_id: Mapped[int] = mapped_column(ForeignKey("bbs_tag.id"),
                                        nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime,
                                                 default=now_utc,
                                                 nullable=False)
...
class CommentLike(Base):
    ...
    id: Mapped[int] = mapped_column(IdType,
                                    primary_key=True,
                                    autoincrement=True)
    comment_id: Mapped[int] = mapped_column(ForeignKey("post_comment.id"),
                                            nullable=False)
    user_id: Mapped[int] = mapped_column(ForeignKey("user_account.id"),
                                         nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime,
...
class ResourcePurchase(Base):
    ...
    buyer_id: Mapped[int] = mapped_column(ForeignKey("user_account.id"),
                                          nullable=False)
    seller_id: Mapped[int] = mapped_column(ForeignKey("user_account.id"),
                                           nullable=False)
