...
router = APIRouter(prefix="/users/me", tags=["wallet"])
...
async def my_wallet(db: DbSession, user: CurrentUser):
    ...
    updated_at = wallet.updated_at or now_utc()
    return success({
...
@router.get("/ledger",
            response_model=ApiEnvelope[list[WalletLedgerItemResponse]])
async def my_ledger(db: DbSession, user: CurrentUser):
    rows = (await db.scalars(
        select(WalletLedger).where(WalletLedger.user_id == user.id).order_by(
            desc(WalletLedger.created_at)).limit(100))).all()
    return success([
        model_dict(
            row,
            [
